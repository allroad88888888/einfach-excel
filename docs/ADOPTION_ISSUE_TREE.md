...
站点、demo、CI、双语文档与双引擎 parity 均存在，但 npm 上仍没有经过离体验证的可安装包。发布口径的五项裁决已经落定（见上节），`@einfach/solid-excel` 的产物形态、内部依赖与 WASM 交付则仍待实施；因此 **AD-100 仍是发布与上手路径的硬阻塞**。

## 当前汇总

| 总叶子 | 完成并独立验收 |               验收契约阻塞 | 未开始 |        其中待决策 |
| -----: | -------------: | -------------------------: | -----: | ----------------: |
|    235 |            131 | 1（AD-505 验收契约待明确） |    103 |                 0 |
...
- AD-000 对外采用与推广（总控）
  - [**AD-100 让人装得上（发布链路）**](adoption-issues/AD-100-publish-pipeline.md) —— 硬阻塞；口径已裁决（ADR 0014~0018）；AD-122、AD-123、AD-125、AD-126、AD-128 完成；其余尚未闭环
