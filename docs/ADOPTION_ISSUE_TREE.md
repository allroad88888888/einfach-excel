...
**首发已完成（2026-08-17）**：五个包（`spreadsheet-ui-core`、`spreadsheet-ui-styles`、`excel-core-ts`、`excel-wasm`、`solid-excel`）均以 `0.1.0` 上线 registry.npmjs.org，并经仓外真实 `npm install` 与运行时/类型探针验证。「能否安装」的未知数就此消除，**AD-100 从硬阻塞降级为质量收尾**（release notes、稳定性声明、四框架冒烟、失败路径走查）；AD-200（五分钟上手）的阻塞解除，可以开工。
...
AD-505 的验收契约（路由 `/?bench=1`、dev 即启、registry 单点登记、无场景显式提示）已由执行方定义并实现，阻塞解除。原 AD-100/D1~D5 五项待决策已裁决为 ADR 0014~0018。

## 树

- AD-000 对外采用与推广（总控）
  - [**AD-100 让人装得上（发布链路）**](adoption-issues/AD-100-publish-pipeline.md) —— **首发已完成（2026-08-17，五包 0.1.0 上线并经离体验证）**；AD-101~130、AD-132~143 完成（发布链路全组收官,仅 AD-131 待维护者重定义）
  ...
  - [**AD-200 让人五分钟内跑起来**](adoption-issues/AD-200-onboarding.md) —— AD-201~203、AD-205~211 完成（最小示例 20 行实测可跑、Quickstart、五框架配方、UI-core-only 路径）；剩 AD-221（dev CJS 治本,走查立叶,issue #12）
  ...
  - [**AD-400 定位与可信度**](adoption-issues/AD-400-positioning.md) —— AD-401~420 全组完成
  - [**AD-500 可验证的性能证据**](adoption-issues/AD-500-performance.md) —— AD-501~517 全组完成（基准页 `/?bench=1` 三场景实测归档,见 docs/BENCHMARK.md）
  - [**AD-600 内容与渠道**](adoption-issues/AD-600-content-channels.md) —— AD-601、AD-602、AD-606~611 完成（选题/渠道映射定稿,文章一二三成稿配图入库 docs/content/）；余项进行中或待发布决策
  - [**AD-700 承接进来的人**](adoption-issues/AD-700-community.md) —— AD-701~706、AD-708~713 完成；剩 AD-707（good first issue 标记）、AD-714（首月复盘,时间门控）
  - [**AD-800 规模能力与技术差异化**](adoption-issues/AD-800-scale-differentiation.md) —— AD-801~817、AD-819~829 完成;剩 AD-818（条件未满足）、AD-830（引擎 7.34M 导入边界,实测立叶,issue #13）
...
| AD-119 | 内部依赖可解析            | `8aadfff`、`1b08a6a`                                                                   |
...
| AD-132 | 发布 workflow             | `3be70d7`、`93f7492`、`6ddc17a`（实跑 run 32003823855，五包上线）                      |
| AD-133 | 发布流程文档化            | `3be70d7`                                                                              |
| AD-134 | 稳定性声明                | `de98cf6`                                                                              |
| AD-135 | 首条 release notes        | GitHub Releases（五 tag 于 `6ddc17a`，主 note 在 solid-excel@0.1.0）                   |
| AD-136 | 本地 registry             | `1b08a6a`                                                                              |
| AD-137 | 全链路 dry-run            | `1b08a6a`                                                                              |
| AD-138 | Vite 仓外冒烟             | `c423d1a`                                                                              |
| AD-139 | webpack 仓外冒烟          | `8e05c1d`                                                                              |
| AD-140 | Next 仓外冒烟             | `af43c2d`                                                                              |
| AD-141 | Nuxt 仓外冒烟             | `3a0741a`                                                                              |
| AD-142 | 失败路径可读性走查        | `e12b416`                                                                              |
| AD-201 | 最小示例(20 行实测)      | `46b2590`                                                                              |
| AD-202 | 最小示例可运行性验证      | `46b2590`                                                                              |
| AD-203 | Quickstart                | `46b2590`                                                                              |
| AD-205 | UI-core-only 路径         | `46b2590`                                                                              |
| AD-206 | Vite 配方                 | `46b2590`                                                                              |
| AD-207 | webpack 配方              | `46b2590`                                                                              |
| AD-208 | Next 配方                 | `46b2590`                                                                              |
| AD-209 | Nuxt 配方                 | `46b2590`                                                                              |
| AD-210 | Astro 配方                | `46b2590`                                                                              |
| AD-211 | 配方入口页                | `46b2590`                                                                              |
| AD-212 | StackBlitz 模板入口       | `158406c`                                                                              |
| AD-213 | CodeSandbox 模板入口      | `158406c`                                                                              |
| AD-214 | degit starter 模板(实测)  | `158406c`                                                                              |
| AD-215 | 模板与文档联动            | `158406c`                                                                              |
| AD-216 | 三方法最小后端示例        | `158406c`                                                                              |
| AD-217 | 可选端口降级演示          | `158406c`                                                                              |
| AD-218 | 自定义后端教程            | `158406c`                                                                              |
| AD-204 | 站点 getting-started 对齐 | `58adab6`                                                                               |
| AD-219 | 首次体验走查(零上下文)    | `b1a1591`                                                                              |
| AD-220 | 走查结论回写(7/9 落地)    | `b1a1591`                                                                              |
| AD-605 | 架构图素材                | `58adab6`                                                                               |
| AD-601 | 选题清单定稿              | `46b2590`                                                                              |
| AD-602 | 受众与渠道映射            | `46b2590`                                                                              |
| AD-606 | 文章一初稿                | `46b2590`                                                                              |
| AD-607 | 文章一定稿与配图          | `46b2590`                                                                              |
| AD-608 | 文章二初稿                | `46b2590`                                                                              |
| AD-609 | 文章二定稿与配图          | `46b2590`                                                                              |
| AD-610 | 文章三初稿                | `46b2590`                                                                              |
| AD-611 | 文章三定稿与配图          | `46b2590`                                                                              |
| AD-612 | 文章四初稿                | `c3750e2`                                                                              |
| AD-613 | 文章四定稿与配图          | `c3750e2`                                                                              |
| AD-614 | 文章五初稿                | `c3750e2`                                                                              |
| AD-615 | 文章五定稿与配图          | `c3750e2`                                                                              |
| AD-617 | 英文渠道清单              | `7277309`                                                                              |
| AD-618 | 中文渠道清单              | `7277309`                                                                              |
| AD-619 | 发布节奏表                | `7277309`                                                                              |
| AD-620 | Show HN 检查单            | `7277309`                                                                              |
| AD-621 | Show HN 正文              | `7277309`                                                                              |
| AD-143 | worker 启动失败错误面     | `77ad63d`                                                                              |
| AD-127 | ui-core keywords 对齐     | `e64ac7b`                                                                              |
| AD-413 | 维护者与响应预期          | `e64ac7b`                                                                              |
| AD-414 | GitHub topics 对齐        | 线上核验（存量交付,2026-08-17 复核）                                                   |
| AD-415 | 仓库 description 对齐     | 线上核验（存量交付,2026-08-17 复核）                                                   |
| AD-703 | Discussions 开启          | 线上操作（2026-08-17）                                                                 |
| AD-704 | SECURITY.md               | `e64ac7b`                                                                              |
| AD-711 | 响应节奏声明              | `e64ac7b`                                                                              |
| AD-712 | 多包版本协同说明          | `de98cf6`、`e64ac7b`                                                                   |
...
| AD-505 | 基准页骨架(契约四要素)    | `4d4da6f`                                                                              |
| AD-506 | 滚动基准                  | `4d4da6f`                                                                              |
| AD-507 | 重算基准                  | `4d4da6f`                                                                              |
| AD-508 | 首屏基准                  | `4d4da6f`                                                                              |
| AD-510 | 结果导出                  | `4d4da6f`                                                                              |
| AD-515 | 结果页与复跑脚本公开      | `4d4da6f`                                                                              |
...
| AD-808 | 规模交互实测(6M 档)       | `77a1b6b`                                                                              |
| AD-809 | 规模重算实测(7M 档)       | `77a1b6b`                                                                              |
| AD-812 | 规模契约固化(PV-36/37)    | `77a1b6b`                                                                              |
| AD-828 | 远程后端视口浏览演示      | `6fa05a1`                                                                              |
| AD-829 | 规模传输叙事收口          | `6fa05a1`                                                                              |
...
- ~~AD-100 是唯一发布硬阻塞~~ 首发已完成（2026-08-17）：「能否安装」的未知数已消除，AD-200 → AD-600 的链路解锁；AD-100 余项是发布质量收尾，不再阻塞下游。
