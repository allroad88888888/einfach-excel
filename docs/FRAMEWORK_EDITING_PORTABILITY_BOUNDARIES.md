...
`excel/solid-excel/src/provider/edit-dispatch.ts:25–186` 也不读取 Solid 或 DOM：它把 Core 编辑命令接到显式 `Store`、后端、历史记录器和投影刷新参数。因此其命令编排可作为框架中立候选来审视；不过它当前仍使用 Solid provider 目录中的组合依赖，本文不据此改变 AD-312 的现有适配器边界。
