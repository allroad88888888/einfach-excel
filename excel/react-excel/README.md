...
删除暂不支持撤销；当前表删除后切换到邻表，静态公式引用变为 `#REF!`，重建同名表不会恢复旧引用。
...
工作表标签已接入上述管理操作；缩放区域仍主要负责界面展示。历史记录等完整 Excel 功能尚未接入。
...
- 当前覆盖单格输入、格式、清除内容/格式/全部、行高/列宽/恢复默认尺寸、粘贴及同表剪切。
- 历史由 Rust 保存，最多 50 步及约 32 MiB 估算预算；不跨刷新持久化。初始化数据不计入历史。
- 修改 E2 后切换 Summary，再撤销，可看到公式回到原值，当前工作表不会跳回 Sales Orders。
- 复制、切表保留历史；粘贴新增一步历史。创建/改名/删除/移动工作表目前仍清空历史并解释原因。
  工作表结构撤销尚未接入。新修改会丢弃重做分支，无变化的提交不会。
- 普通粘贴、七种选择性模式、转置、跳过空白、平铺均可撤销；同表剪切的一步撤销会恢复源格、
