export const RUST_DEMO_DATA_ROW_COUNT = 1_000
export const RUST_DEMO_COLUMN_COUNT = 8
export const RUST_DEMO_CELL_COUNT =
  (RUST_DEMO_DATA_ROW_COUNT + 1) * RUST_DEMO_COLUMN_COUNT
export const RUST_DEMO_IMPORT_CHUNK_SIZE = 500

export type RustDemoImportCell =
...
function textCell(row: number, col: number, value: string): RustDemoImportCell {
  return { sheet: 0, row, col, kind: 'text', value }
}

function numberCell(row: number, col: number, value: number): RustDemoImportCell {
  return { sheet: 0, row, col, kind: 'number', value }
}

function createDataRow(dataRow: number): RustDemoImportCell[] {
  const row = dataRow + 1
...
export function createRustDemoImportChunks(
  chunkSize = RUST_DEMO_IMPORT_CHUNK_SIZE,
): RustDemoImportCell[][] {
  if (!Number.isInteger(chunkSize) || chunkSize < 1) {
    throw new Error('Rust demo import chunk size must be a positive integer.')
  }

  const cells: RustDemoImportCell[] = [
    ...HEADERS.map((header, col) => textCell(0, col, header)),
    ...Array.from({ length: RUST_DEMO_DATA_ROW_COUNT }, (_, row) => createDataRow(row)).flat(),
  ]
  const chunks: RustDemoImportCell[][] = []
