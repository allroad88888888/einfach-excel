...
import {
  DEMO_GRID_ROW_HEIGHT,
  DEMO_GRID_WINDOW_ROW_COUNT,
} from '../demo/use-demo-grid-window'
...
function valueAt(row: number, col: number): string {
  if (row === 0) return DEMO_COLUMNS[col]?.label ?? ''
...
describe('Rust demo projection window', () => {
  it('requests a bounded orders window and only mounts that projection', async () => {
    ...
    const projectedCellCount = DEMO_GRID_WINDOW_ROW_COUNT * DEMO_COLUMNS.length
  ...
  it('reaches the last record while preserving absolute selection coordinates', async () => {
    ...
    const scrollHeight = (DEMO_SHEET_ROW_COUNT + 1) * DEMO_GRID_ROW_HEIGHT
    const clientHeight = 1_200
    const maxScrollTop = scrollHeight - clientHeight
    expect(clientHeight).toBeGreaterThan(924)
    expect(Math.floor(maxScrollTop / DEMO_GRID_ROW_HEIGHT)).toBeLessThan(
      DEMO_SHEET_ROW_COUNT - DEMO_GRID_WINDOW_ROW_COUNT,
    )
    ...
    expect(controlled.requests[1]?.window).toEqual({
      rowStart: DEMO_SHEET_ROW_COUNT - DEMO_GRID_WINDOW_ROW_COUNT,
      rowEnd: 1_000,
      colStart: 0,
      colEnd: 7,
    })
    ...
    expect(document.querySelectorAll('td')).toHaveLength(
      DEMO_GRID_WINDOW_ROW_COUNT * DEMO_COLUMNS.length,
    )
  })
