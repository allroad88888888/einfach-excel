...
impl Workbook {
    pub fn remove_sheet(&mut self, idx: usize) -> Option<Sheet> {
        ...
        if idx >= self.sheets.len() || self.sheets.len() <= 1 {
            return None;
        }
        let rewrites = self.sheet_ref_rewrites(&self.names[idx], None);
        ...
        store.batch(|_| {
            ...
            for (original_index, addr, text) in rewrites {
                if original_index == idx {
                    continue;
                }
                let index = if original_index > idx {
                    original_index - 1
                } else {
                    original_index
                };
                self.set_formula(index, &addr.to_string_repr(), &text);
            }
            removed = Some(sheet);
        });
        removed
