...
//! # 为什么公式里一个 `1E308` 都不写
//!
//! **本引擎的解析器不认科学计数字面量**：`=1E2` 在这里是 `#VALUE!`（`1` 后面
//! 跟了个叫 `E2` 的单元格引用），Excel 与 TS 参考引擎都给 `100`。那是一条独立
//! 的、先于本待办存在的分歧，住在 `formula.rs` 的词法层。用 `10^308` 这类幂
//! 表达式绕开它，是为了让这份文件只量溢出闸门一件事 —— 否则每一行都会先红在
//! 词法上，量不到闸门。那条词法分歧另立待办。
//!
//! # 为什么不碰非规格化数
//!
