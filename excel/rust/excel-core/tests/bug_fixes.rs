...
//! Bug 2 — unbounded range (A:A) in VLOOKUP stays sparse and returns a value
...
/// Bug 2: VLOOKUP with an unbounded full-column range `A:A` uses the sparse
/// lookup materializer instead of rejecting the whole-axis sentinel.
#[test]
fn vlookup_unbounded_column_range_returns_the_matching_value() {
    let mut wb = Workbook::new();
    // Put a value in A1 so the sheet is non-empty.
    wb.set_cell(0, "A1", Value::Number(1.0));
    wb.set_cell(0, "B1", Value::Number(42.0));
    // A:B has a full-column sentinel row bound. It must not walk all rows.
    wb.set_formula(0, "C1", "=VLOOKUP(1,A:B,2,FALSE)");
    assert_eq!(wb.get_cell("Sheet1", "C1"), Value::Number(42.0));
}
