> **未兑现的 backlog。** 2026-07-30 从已归档的 `archive/online-excel-parity/` 提升到 `docs/`：
> 上轮 remote-call 实现被撤回后至今未重做（`grep -rn "remote-call" excel/rust/*/src` 零命中），
> 因此本计划仍然有效，不是历史记录。
...
- 本分支并回主线前不开工（W4 排期，见 `archive/online-excel-parity/SALVAGE_PLAN_REVISIONS.md` §八）；
- 工作量预估 3–5 天全链路;
