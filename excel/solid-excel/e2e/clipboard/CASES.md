...
| CB-15 | 大范围粘贴角点投影（真 Worker）             | 编辑 B2/E2/B8/E8 后，B2:E8 → G2；名称框跳至 G2/J2/G8/J8 | 四角均进入 DOM 且显示源矩形对应值      | ✅ 本轮    | large-range-paste-projection（wasm+ts）                                                                |
| CB-16 | Ctrl/⌘+Alt+V 打开真后端 Paste Special       | Worker 内部 copy → Ctrl/⌘+Alt+V                         | TS/WASM 对应对话框可见                 | ✅ 本轮    | paste-special-worker-shortcut（wasm+ts）                                                               |
| CB-17 | 粘贴驱动 status-last-command                | copy→paste 看状态栏                                     | 含 paste/clipboard                     | ✅ 存量    | audit-clipboard #6                                                                                     |
| CB-18 | 粘贴写入 history timeline                   | copy→paste 看时间线                                     | 含 clipboard/cells.import 条目         | ✅ 存量    | audit-clipboard #7                                                                                     |
