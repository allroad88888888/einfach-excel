...
- Replace/Replace-All 经后端 port 获得确认后，以宿主提供的 history capability 写入一个
  `cell.set-input` 事务；Replace All 的一次撤销会还原该页所有已替换命中，见 FR-14。
...
| FR-14 | Replace All 是单个 undo 步                    | 240→888 后一次 undo 应双格还原                             | undo 一步全还原                                                | ✅ 已修复  | replace-all-limits.spec.ts #"replace-all is one undo step: a single undo restores every match"（9acf973） |
