...
- 锁定拦截的用户可见形态是**编辑器拒绝打开**（dblclick 无 input、type-to-edit 无效）
  ＋ Grid 内的 `role="alert"` 恢复提示（可关闭，或对当前单元格打开 Unlock range）
  ＋**工具栏格式按钮禁用**（`isProtectionGated`）。
...
| PR-05 | 锁定编辑拦截的可见恢复提示          | protect → dblclick B4 → alert Unlock range → confirm → retry                           | 无 input 时显示 alert；dialog 定位 B4；confirm 后 B4 可编辑且 alert 清除 | ✅ 本轮    | vnext-protection-real-backend #"a locked direct edit gives a visible range-unlock recovery path"                                   |
