...
import type { WorkerWorkbookRuntimeResources } from './worker-runtime-resources'
import {
  assertMethod,
  assertSheet,
  normalizeAddr,
...
function debugCounters(
  ...
  runtimeResources: WorkerWorkbookRuntimeResources,
  ...
  const handles = runtimeResources.sessionHandles.sessionHandleCounts()
  return {
...
function resetWorkbook(
  ...
  runtimeResources.resetForNewWorkbook(currentWorkbook())
  return replaceWorkbook(sheets)
}

export function createWorkbookCommandHandler(
  runtimeResources: WorkerWorkbookRuntimeResources,
): WorkerCommandHandler {
  return (id, msg, workbook) => {
    let wb = workbook
    switch (msg.cmd) {
      case 'initWorkbook':
        wb = resetWorkbook(
          runtimeResources,
          Array.isArray(msg.sheets) ? msg.sheets.map(String) : undefined,
        )
      ...
      case 'removeSheet': {
        ...
        if (removed) runtimeResources.sessionHandles.invalidateSheetIndexedHandles(wb)
      ...
      case 'moveSheet': {
        ...
        if (from !== to) runtimeResources.sessionHandles.invalidateSheetIndexedHandles(wb)
      ...
      case 'undefineName':
        ...
        throw Object.assign(
          new Error(
            'LAMBDA name bindings are not supported by the WASM runtime — use the TS backend (?backend=ts).',
          ),
      ...
      case 'debugFormulaEvalCount':
        ...
        return true
      case 'debugCounters':
        postResponse(id, debugCounters(wb, runtimeResources))
        return true
      default:
        return false
    }
