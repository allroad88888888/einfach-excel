...
type GridContextMenuRuntime = GridRuntimeBase &
  ...
  GridSelectionContextPort &
  GridMergeAnchorPort

export function installGridContextMenu(runtime: GridContextMenuRuntime) {
  const { props, store, selectionSnapshot, editingSession, getSelectionRangeContaining, dom } =
    runtime
  ...
  function getKeyboardContextMenuInput(): MenuOpenInput | null {
    ...
    const activeCellElement =
      ...
      runtime.findMergeAnchorCovering(snapshot.activeCell.row, snapshot.activeCell.col)?.el ??
      null
      ...
      case 'range': {
        ...
        input = {
          ...
          target: single
            ...
            : { kind: 'range', sheetId: props.sheetId, range: snapshot.range },
        }
        break
      }
      case 'row':
        ...
        input = {
          surface: 'header',
          target: { kind: 'row', sheetId: props.sheetId, rowIndex: snapshot.selection.rowFocus },
        }
        break
      case 'column':
        anchorElement =
        ...
        input = {
          surface: 'header',
          target: { kind: 'column', sheetId: props.sheetId, colIndex: snapshot.selection.colFocus },
        }
        break
      case 'all':
        anchorElement =
          gridRoot.querySelector<HTMLElement>('.spreadsheet-grid-corner') ?? activeCellElement
        input = { surface: 'header', target: { kind: 'all', sheetId: props.sheetId } }
        break
    }
  ...
  function targetFallsWithinSingleAxisSelection(
    ...
    if (target.kind === 'row' && region.kind === 'row')
      return (
        target.row >= Math.min(region.rowAnchor, region.rowFocus) &&
        target.row <= Math.max(region.rowAnchor, region.rowFocus)
      )
    if (target.kind === 'column' && region.kind === 'column')
      return (
        target.col >= Math.min(region.colAnchor, region.colFocus) &&
        target.col <= Math.max(region.colAnchor, region.colFocus)
      )
    return false
  }
  ...
  function openContextMenu(
    ...
    store.setter(openMenuAtom, {
      ...
      target:
        target.kind === 'cell'
          ? { kind: 'cell', sheetId: props.sheetId, cell: { row: target.row, col: target.col } }
          : target.kind === 'range'
            ? { kind: 'range', sheetId: props.sheetId, range: target.range }
            : target.kind === 'row'
              ? { kind: 'row', sheetId: props.sheetId, rowIndex: target.row }
              : target.kind === 'column'
                ? { kind: 'column', sheetId: props.sheetId, colIndex: target.col }
                : { kind: 'all', sheetId: props.sheetId },
  ...
  function getCellContextTarget(
    ...
    if (range && (range.rowStart !== range.rowEnd || range.colStart !== range.colEnd))
  ...
  function getCellCoordFromPoint(event: PointerEvent): CellCoord | null {
    const cell = document
      ...
      ?.closest?.('td.spreadsheet-grid-cell') as HTMLElement | null
    const gridRoot = dom.gridRoot()
  ...
  function getFillPreviewRange(): CellRange | null {
    ...
    return session.status === 'active' &&
      ...
      ? session.interaction.previewRange
      : null
  }
  ...
  return installGridFeature(runtime, {
    ...
    targetFallsWithinSingleAxisSelection,
    openContextMenu,
