...
type GridKeyboardControllerRuntime = GridRuntimeBase &
  ...
  Pick<
    ...
    'getDataEdgeDirection' | 'moveSelectionToDataEdge' | 'startEditingCell'
  > &
...
export function installGridKeyboardController(runtime: GridKeyboardControllerRuntime) {
  ...
  async function handleGridKeyDown(event: KeyboardEvent) {
    ...
    if (
      ...
      (await moveSelectionToDataEdge(event, dataEdgeDirection))
    ) {
      store.setter(scrollToCellAtom, {
        coord: store.getter(selectionSnapshotAtom).activeCell,
      })
      return
    }
    ...
    switch (intent.type) {
      ...
      case 'selection.move':
        event.preventDefault()
        if (intent.scroll) store.setter(scrollToCellAtom, { coord: intent.scroll.target })
        return
      ...
      case 'filterSort.reapply':
        ...
        await store.setter(reapplyFilterAtom, {
          source: backend,
          entrypoint: 'menu-bar',
          refreshProjection: (sheetId: string) => refreshVisibleProjection(store, backend, sheetId),
        })
        return
      case 'clipboard.copy':
        event.preventDefault()
      ...
      case 'clipboard.copyAs': {
        ...
        if (snapshot.selection.sheetId === props.sheetId)
          await dispatchCopyAs(store, backend, { sheetId: props.sheetId, range: snapshot.range })
        return
      }
