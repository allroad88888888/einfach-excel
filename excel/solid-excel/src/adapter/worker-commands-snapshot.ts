...
import type { WorkerSessionHandleRegistry } from './worker-session-registry'
import {
  assertMethod,
  assertSheet,
  normalizeSnapshot,
...
export function createSnapshotCommandHandler(
  sessionHandles: WorkerSessionHandleRegistry,
  ...
  return (id, msg, wb) => {
    switch (msg.cmd) {
      ...
      case 'restorePersistenceV1': {
        ...
        sessionHandles.resetSessionHandles(wb)
