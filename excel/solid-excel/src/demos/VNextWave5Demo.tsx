...
import { workbenchBackend, workbenchNamedRangeCapabilityPort, workbenchSheets, workbenchViewport } from './workbench-seed'
...
function VNextWave5Workbook() {
  ...
  const activeSheetId = () => workspace().activeSheetId ?? workbenchSheets[0].id
  ...
  onMount(() => {
    const mountSheetId = store.getter(workspaceSessionAtom).activeSheetId ?? workbenchSheets[0].id
    if (!store.getter(workspaceSessionAtom).activeSheetId) {
      store.setter(setWorkspaceActiveSheetAtom, { sheetId: workbenchSheets[0].id })
    }
      ...
      <div class="vnext-demo-body">
        <div class="vnext-demo-main">
          <Show keyed when={activeSheetId()}>
            {(sheetId) => (
              <SpreadsheetGrid sheetId={sheetId} viewport={workbenchViewport} data-testid="wave5-grid" />
            )}
          </Show>
          <div class="vnext-demo-bottom-row">
            <SpreadsheetSheetTabs sheets={workbenchSheets} data-testid="wave5-sheet-tabs" />
          </div>
...
export function SpreadsheetWorkbenchDemo() {
  return (
    <div class="demo-page vnext-demo" data-testid="wave5-demo">
      ...
      <SpreadsheetUiProvider
        backend={backend}
        namedRangeCapabilityPort={workbenchNamedRangeCapabilityPort}
      >
        <VNextWave5Workbook />
      </SpreadsheetUiProvider>
    </div>
