 ...
 * # 为什么一个 `1E308` 都不写
 *
 * 起初是**被迫**的：Rust 引擎的解析器当时不认科学计数字面量，`=1E2` 在它上面
 * 是 `#VALUE!`（`1` 后面跟了个叫 `E2` 的单元格引用），而 TS 引擎给 `100`。那
 * 条词法分歧**现在已经修好**，单列一类钉在 `cross-engine-parity-scientific.ts`。
 *
 * 但下面**仍然一个都不改成 `1E308`**，理由变了：`10^308` 这类幂表达式同时
 * 覆盖了 `^` 自己的溢出路径（它是唯一一个此前就有闸门的运算符）。换成字面量
 * 会把这一路覆盖白白丢掉 —— 那是削弱而不是简化。
 */
