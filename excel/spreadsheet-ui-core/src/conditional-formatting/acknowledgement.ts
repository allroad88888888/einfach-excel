...
export function snapshotAcknowledgement(
  ...
  try {
    ...
    if (typeof value.sheetId !== 'string' || value.sheetId !== ticket.sheetId)
      return {
        acknowledgement: null,
...
export function snapshotRulesResult(
  value: unknown,
  ticket: ConditionalFormatRulesResponseTicket,
): RulesResultSnapshot {
  if (!isObjectRecord(value))
...
export function copyMutationRequest(
  ...
  return request.kind === 'remove-conditional-format-rule'
    ? { ...request }
    : { ...request, scope: copyScope(request.scope), rule: copyRule(request.rule) }
}
