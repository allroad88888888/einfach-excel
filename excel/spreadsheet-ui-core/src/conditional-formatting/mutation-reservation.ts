...
import {
  resolveScopeTarget,
  resolveSheetTarget,
  resolvedTargetAuthorityIsCurrent,
} from './target-domain'
import type {
...
export const reserveConditionalFormatMutationLaunchAtom = atom(
  ...
  (
    ...
    if (
      load.phase === 'pending' &&
      load.sheetId === editor.sheetId &&
      load.sessionId === editor.sessionId
    ) {
      return releaseCapture(
        get,
        set,
        capture,
        'Conditional formatting rules are still loading for the active sheet',
      )
    }
    ...
    if (editor.sheetId !== sheetTarget.sheetId || cache.sheetId !== sheetTarget.sheetId) {
      return releaseCapture(
        get,
        set,
        capture,
        'Conditional formatting rule selection does not belong to the active sheet',
      )
    }
    if (ruleId !== null && !cache.rules.some((rule) => rule.id === ruleId)) {
      return releaseCapture(
        get,
        set,
        capture,
        'Conditional formatting selected rule is no longer available on the active sheet',
      )
    }
