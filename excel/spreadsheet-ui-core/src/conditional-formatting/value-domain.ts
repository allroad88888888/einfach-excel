...
export function freezeRulesLoadState(
  state: ConditionalFormatRulesLoadState,
): ConditionalFormatRulesLoadState {
...
export function defaultRuleForKind(kind: ConditionalFormatRuleKind): ConditionalFormatRule {
  switch (kind) {
    case 'cell-value':
      return { kind, operator: 'gt', value: '0', format: { bgColor: '#fef3c7' } }
    case 'formula':
      return { kind, formula: '=TRUE()', format: { bgColor: '#fef3c7' } }
    case 'data-bar':
      return { kind }
    case 'color-scale':
      return { kind, minColor: '#ff0000', maxColor: '#00ff00' }
    case 'top-bottom':
      return { kind, direction: 'top', count: 10, format: { bgColor: '#fef3c7' } }
  }
}
...
export function closeEditorState(
  previous: ConditionalFormatEditorState,
): ConditionalFormatEditorState {
