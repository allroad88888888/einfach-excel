...
import type { FilterSortEntrypointTarget } from './types'
import type { FilterSortEntrypointTicket, FilterSortMutationTicket } from './internal-types'
import {
...
export function resolveFilterSortEntrypointTarget(get: Getter): FilterSortEntrypointTarget | null {
  ...
  return !sheetId || !Number.isSafeInteger(colIndex) || colIndex < 0
    ? null
    : Object.freeze({ sheetId, colIndex })
}
export const sameFilterSortTarget = (
  left: FilterSortEntrypointTarget | null,
  right: FilterSortEntrypointTarget | null,
): boolean => left?.sheetId === right?.sheetId && left?.colIndex === right?.colIndex
export function mutationTicketAuthorityIsCurrent(
  get: Getter,
  ticket: FilterSortMutationTicket,
): boolean {
  const dropdown = get(filterDropdownBackingAtom)
  const draft = get(filterSortDraftBackingAtom)
  const lifecycle = get(filterSortLifecycleBackingAtom)
  return (
    dropdown.status === 'open' &&
    dropdown.sheetId === ticket.sheetId &&
    dropdown.colIndex === ticket.colIndex &&
    get(filterSortSessionIdBackingAtom) === ticket.sessionId &&
