...
import {
  ...
  filterSortSessionIdBackingAtom,
...
const hasActiveTicket = (get: Parameters<Parameters<typeof atom>[1]>[0]) =>
  get(activeFilterSortMutationAtom) !== null || get(activeFilterSortEntrypointAtom) !== null
export const issueFilterSortSyncTicketAtom = atom(null, (get, set) => {
  const next = nextFilterSortRequestId(get(filterSortSyncTicketAtom))
  if (next === null) return get(filterSortSyncTicketAtom)
...
export const setFilterSortAtom = atom(
  ...
  (get, set, { sheetId, state }: { sheetId: string; state: FilterSortState }) =>
    set(
      filterSortStateBackingAtom,
      stateStoreWith(get(filterSortStateBackingAtom), sheetId, state),
    ),
)
...
export const reconcileFilterSortRulesFromEngineAtom = atom(
  ...
  (
    get,
    set,
    { sheetId, rules }: { sheetId: string; rules: readonly ColumnFilterRule[] },
  ) =>
    set(
      filterSortStateBackingAtom,
      rules.length === 0
        ? stateStoreWithout(get(filterSortStateBackingAtom), sheetId)
        : stateStoreWith(get(filterSortStateBackingAtom), sheetId, { rules }),
    ),
)
...
export const captureFilterSortCapabilityAtom = atom(
  ...
  (get, set, source: FilterSortControllerPort) => {
    ...
    if (
      get(filterSortLifecycleAtom).status === 'blocked' &&
...
export const clearFilterSortAtom = atom(
  ...
  (get, set, sheetId: string) => {
    ...
    const next = stateStoreWithout(get(filterSortStateBackingAtom), sheetId)
    set(filterSortStateBackingAtom, next)
    const nextSession = nextFilterSortSessionId(get(filterSortSessionIdAtom))
    if (nextSession !== null) set(filterSortSessionIdBackingAtom, nextSession)
    const session = nextSession ?? get(filterSortSessionIdAtom)
    set(filterDropdownBackingAtom, CLOSED_FILTER_DROPDOWN_STATE)
    set(filterSortDraftBackingAtom, closedFilterSortDraft(session))
    set(filterSortLifecycleBackingAtom, lifecycleFor('closed', session, null, null))
    set(filterSortErrorBackingAtom, '')
  },
)
clearFilterSortAtom.debugLabel = 'spreadsheet.filterSort.clear'
const clearColumn = atom(
  (get) => get(filterSortStateAtom),
  (get, set, input: { sheetId: string; colIndex: number }) => {
    if (hasActiveTicket(get)) return
    const state = get(filterSortStateAtom)[input.sheetId]
    if (!state) return
    const rules = state.rules.filter((rule) => rule.colIndex !== input.colIndex)
    if (rules.length === state.rules.length) return
    set(
      filterSortStateBackingAtom,
      stateStoreWith(get(filterSortStateBackingAtom), input.sheetId, { rules }),
    )
  },
)
export const clearColumnFilterSortAtom = clearColumn
clearColumnFilterSortAtom.debugLabel = 'spreadsheet.filterSort.clearColumn'
export const clearColumnFilterRulesAtom = clearColumn
...
export const openFilterDropdownAtom = atom(
  ...
  (get, set, { sheetId, colIndex }: { sheetId: string; colIndex: number }) => {
    if (hasActiveTicket(get) || !sheetId || !Number.isSafeInteger(colIndex) || colIndex < 0) return
...
export const closeFilterDropdownAtom = atom(
  ...
  (get, set) => {
    if (!get(filterSortCanCloseAtom)) return
    const next = nextFilterSortSessionId(get(filterSortSessionIdAtom))
    if (next !== null) set(filterSortSessionIdBackingAtom, next)
    const session = next ?? get(filterSortSessionIdAtom)
    set(filterDropdownBackingAtom, CLOSED_FILTER_DROPDOWN_STATE)
    set(filterSortDraftBackingAtom, closedFilterSortDraft(session))
    set(filterSortLifecycleBackingAtom, lifecycleFor('closed', session, null, null))
    set(filterSortErrorBackingAtom, '')
...
export const updateFilterSortAvailableValuesAtom = atom(
  ...
  (get, set, input: UpdateFilterSortAvailableValuesInput) => {
    ...
    set(
      filterSortDraftBackingAtom,
      snapshotFilterSortDraft({ ...draft, availableValues, selectedValues }),
    )
  },
)
