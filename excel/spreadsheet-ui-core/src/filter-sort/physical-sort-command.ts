...
import { recordPhysicalSortHistory } from './acknowledgement-domain'
import {
  entrypointTicketAuthorityIsCurrent,
  resolveFilterSortEntrypointTarget,
} from './authority-domain'
import {
  activeFilterSortEntrypointAtom,
  activeFilterSortMutationAtom,
  filterSortEntrypointOperationIdStateAtom,
...
import {
  ...
  captureRunPhysicalSortInput,
  classifyPhysicalSortAcknowledgement,
  isValidSortRange,
  physicalSortDiagnosticBackingAtom,
  physicalSortRejectionMessage,
  sortRangeCapabilityBackingAtom,
...
export const runPhysicalSortAtom = atom(
  ...
  async (get, set, input: RunPhysicalSortInput): Promise<void> => {
    if (
      ...
      get(filterDropdownAtom).status === 'open'
    )
      return
    const captured = captureRunPhysicalSortInput(input)
    if (get(activeFilterSortEntrypointAtom) !== null || get(activeFilterSortMutationAtom) !== null)
      return
    ...
    const available = typeof captured.transport === 'function'
    set(sortRangeCapabilityBackingAtom, available)
    const explicit = captured.target.kind === 'explicit' ? captured.target.value : null
    const target =
      captured.target.kind === 'selection' ? resolveFilterSortEntrypointTarget(get) : explicit
    const range = captured.range.kind === 'range' ? captured.range.value : null
    const rangeValid = range !== null && isValidSortRange(range)
    const columnInRange =
      rangeValid &&
      target !== null &&
    ...
    const payloadValid =
      ...
      typeof captured.historyEntryRecorder === 'function' &&
      typeof captured.refreshProjection === 'function' &&
      captured.timeoutMs !== null
    const rejection = !available
      ? 'unsupported'
      : !payloadValid || !directionValid || !entrypointValid
        ? 'invalid-payload'
        : target === null
          ? 'empty-keys'
          : !rangeValid
    ...
    if (
      ...
      set(
        ...
        entrypointStateFor('blocked', {
          entrypoint: entrypointValid ? captured.entrypoint : null,
          target,
          direction: directionValid ? captured.direction : null,
          attempt: 1,
          error: message,
        }),
      )
    ...
    const request = Object.freeze({
      kind: 'sort-range' as const,
      sheetId: target.sheetId,
      range,
      keys: Object.freeze([Object.freeze({ col: target.colIndex, direction: captured.direction })]),
      excludedRows: Object.freeze(buildSortExcludedRows(get, target.sheetId, range)),
      requestId,
    })
    const attempt = nextEntrypointAttempt(
    ...
    const ticket: PhysicalSortTicket = Object.freeze({
      ...
      selectionWitness: targetAuthority === 'selection' ? get(selectionAuthorityWitnessAtom) : null,
    ...
    const { owns, markUnknown, releaseUnsent, unknown } = createPhysicalSortTicketControl(
      get,
      set,
      ticket,
    )
    const authority = () => entrypointTicketAuthorityIsCurrent(get, ticket)
    ...
    if (transport.kind === 'rejected') {
      ...
      markUnknown(errorMessage(transport.error))
      return
    }
    ...
    if (acknowledgement.kind === 'invalid') {
      markUnknown(FILTER_SORT_ACKNOWLEDGEMENT_ERROR)
      return
    }
    ...
    if (recordPhysicalSortHistory(set, ticket, acknowledgement) === 'rejected') {
      unknown(FILTER_SORT_ACKNOWLEDGEMENT_ERROR)
      return
    }
    if (!owns()) return
    ...
    if (refresh.kind === 'rejected') {
      set(
        filterSortEntrypointStateBackingAtom,
        entrypointStateForTicket('refresh-failed', ticket, refreshFailureError(refresh.error)),
      )
      return
    }
    if (!set(releaseHistoryProducerReservationAtom, ticket.historyReservation)) {
