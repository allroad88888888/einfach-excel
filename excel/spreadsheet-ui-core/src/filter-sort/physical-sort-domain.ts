...
import type { ProjectionRequestId, SortRangeRejectionCode } from '../backend/types'
...
import { PHYSICAL_SORT_CAPABILITY_ERROR } from './constants'
...
export const physicalSortRejectionMessage = (
  code: PhysicalSortDiagnosticCode,
  fallback?: string,
): string => PHYSICAL_SORT_REJECTION_MESSAGES[code] ?? fallback ?? 'Sort could not run.'
export const sortRangeCapabilityBackingAtom = atom(false)
...
export const physicalSortDiagnosticBackingAtom = atom<PhysicalSortDiagnostic | null>(null)
...
export type CapturedRunPhysicalSortInput =
  ...
  | { readonly kind: 'invalid' }
export function captureRunPhysicalSortInput(
  input: RunPhysicalSortInput,
): CapturedRunPhysicalSortInput {
  try {
    ...
    if (rangeValue === null) range = Object.freeze({ kind: 'absent' })
    else if (typeof rangeValue !== 'object') range = Object.freeze({ kind: 'invalid' })
    else {
      const { rowStart, rowEnd, colStart, colEnd } = rangeValue
      range =
    ...
    if (targetValue === undefined) target = Object.freeze({ kind: 'selection' })
    else if (typeof targetValue !== 'object' || targetValue === null)
      target = Object.freeze({ kind: 'invalid' })
    else {
      const { sheetId, colIndex } = targetValue
      target =
        typeof sheetId === 'string' &&
        sheetId.length > 0 &&
        Number.isSafeInteger(colIndex) &&
        colIndex >= 0
          ? Object.freeze({ kind: 'explicit', value: Object.freeze({ sheetId, colIndex }) })
          : Object.freeze({ kind: 'invalid' })
    }
...
export const isValidSortRange = (range: CellRange): boolean =>
...
const sheetHasRules = (get: Getter, sheetId: string): boolean =>
  (get(filterSortStateAtom)[sheetId]?.rules.length ?? 0) > 0
const isRejectionCode = (value: unknown): value is SortRangeRejectionCode =>
  [
    'invalid-range',
    'empty-keys',
    'key-out-of-range',
...
const sameRange = (left: CellRange, right: CellRange) =>
  left.rowStart === right.rowStart &&
  left.rowEnd === right.rowEnd &&
  left.colStart === right.colStart &&
  left.colEnd === right.colEnd
export function classifyPhysicalSortAcknowledgement(
  value: unknown,
  sheetId: string,
  ...
  try {
    ...
    if (result.sheetId !== sheetId || result.requestId !== requestId) return { kind: 'invalid' }
    if (result.applied === false) {
      const { code, message, anchor } = result
      return result.kind !== 'sort-range-not-applied' ||
    ...
    const { movedRows, movedCells, affectedRange, revision } = result
    if (
      result.applied !== true ||
      result.kind !== 'sort-range' ||
      ...
      return { kind: 'invalid' }
    const fields = affectedRange as Record<PropertyKey, unknown>
    const { rowStart, rowEnd, colStart, colEnd } = fields
    if (
      !Number.isSafeInteger(rowStart) ||
      !Number.isSafeInteger(rowEnd) ||
      !Number.isSafeInteger(colStart) ||
    ...
    return !isValidSortRange(range) ||
      !sameRange(range, requestedRange) ||
...
export function buildSortExcludedRows(get: Getter, sheetId: string, range: CellRange): number[] {
  ...
  const clip = (rows: readonly number[]) => {
    for (const row of rows) if (row >= range.rowStart && row <= range.rowEnd) excluded.add(row)
  }
  clip(getHiddenRowsForSheet(get(viewportHiddenAtom), sheetId))
  if (sheetHasRules(get, sheetId))
