...
import type { FilterSortControllerPort, ReapplyFilterInput } from './types'
import {
...
import { classifyFilterSortAcknowledgement } from './acknowledgement-domain'
import {
...
type Captured =
  | {
      kind: 'captured'
      sourceWitness: FilterSortControllerPort
      transport: FilterSortControllerPort['setFilterSort']
      entrypoint: 'toolbar' | 'menu-bar'
      refreshProjection: ReapplyFilterInput['refreshProjection']
      timeoutMs: number
    }
  | { kind: 'invalid' }
function capture(input: ReapplyFilterInput): Captured {
  try {
...
export const reapplyFilterAtom = atom(
  ...
  async (get, set, input: ReapplyFilterInput): Promise<void> => {
    ...
    const acknowledgement = classifyFilterSortAcknowledgement(
      transport.value,
      ticket.target.sheetId,
      ticket.requestId,
      { expectedHistoryRecorded: false, allowAbsentHiddenRowIndices: true },
    )
    if (!owns()) return
    if (!authority()) {
      stale()
    ...
    set(setViewportFilterHiddenRowsAtom, {
      ...
      rows: acknowledgement.hiddenRowIndices,
    })
    set(
      filterSortEntrypointStateBackingAtom,
