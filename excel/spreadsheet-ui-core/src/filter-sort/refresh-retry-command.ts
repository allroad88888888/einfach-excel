...
function capture(input: RetryFilterSortRefreshInput): Captured {
  try {
    const refreshProjection = input.refreshProjection
    const timeoutMs = snapshotTimeoutMs(input.timeoutMs)
    return typeof refreshProjection !== 'function' || timeoutMs === null
...
export const retryFilterSortRefreshAtom = atom(
  ...
  async (get, set, input: RetryFilterSortRefreshInput): Promise<void> => {
    const mutation = get(activeFilterSortMutationAtom)
    const lifecycle = get(filterSortLifecycleAtom)
    if (
      mutation !== null &&
      lifecycle.status === 'refresh-failed' &&
      lifecycle.sessionId === mutation.sessionId &&
      lifecycle.requestId === mutation.requestId
    ) {
      if (!mutationTicketAuthorityIsCurrent(get, mutation)) return
      const captured = capture(input)
      if (
        get(activeFilterSortMutationAtom) !== mutation ||
        get(filterSortLifecycleAtom) !== lifecycle ||
        !mutationTicketAuthorityIsCurrent(get, mutation) ||
      ...
      const refresh = await runBoundedOperation(
        () => captured.refreshProjection(mutation.sheetId),
        captured.timeoutMs,
      )
      if (
        get(activeFilterSortMutationAtom) !== mutation ||
        ...
        return
      if (refresh.kind === 'timeout' || refresh.kind === 'rejected') {
        set(
          filterSortErrorBackingAtom,
          refreshFailureError(
            refresh.kind === 'timeout' ? FILTER_SORT_REFRESH_TIMEOUT_ERROR : refresh.error,
          ),
        )
        set(
          filterSortLifecycleBackingAtom,
      ...
      if (!set(releaseHistoryProducerReservationAtom, mutation.historyReservation)) {
      ...
      if (get(activeFilterSortMutationAtom) !== mutation) return
      if (!mutationTicketAuthorityIsCurrent(get, mutation)) {
      ...
      set(
        filterSortLifecycleBackingAtom,
        lifecycleFor('editing', mutation.sessionId, mutation.sheetId, mutation.colIndex),
      )
      set(activeFilterSortMutationAtom, null)
      return
    }
    const ticket = get(activeFilterSortEntrypointAtom)
    ...
    if (
      !owns() ||
      get(filterSortEntrypointStateBackingAtom) !== state ||
      captured.kind === 'invalid'
    )
      return
    ...
    if (!owns()) return
    const refresh = await runBoundedOperation(
      () => captured.refreshProjection(ticket.target.sheetId),
      captured.timeoutMs,
    )
    if (!owns()) return
    if (refresh.kind === 'timeout' || refresh.kind === 'rejected') {
