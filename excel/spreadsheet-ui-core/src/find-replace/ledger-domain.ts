...
import type {
  ...
  FindReplaceReconciliationTarget,
  PendingMutation,
  SearchTicket,
} from './internal-types'
...
export function settleAttempt(
  ...
  operationId: string,
  status: FindReplaceOperationAttempt['status'],
): FindReplaceOperationAttempt[] {
...
export function reconcileUnknownAttemptsForTarget(
  ledger: readonly FindReplaceOperationAttempt[],
  ticket: SearchTicket,
): FindReplaceOperationAttempt[] {
  return ledger.map((attempt) =>
...
export function attemptBlocksMutationForTarget(
