...
export function copyRange(range: CellRange): CellRange {
  return { ...range }
}
...
export function sameCoord(left: CellCoord, right: CellCoord): boolean {
...
export function copyQuery(query: FindReplaceQuery): FindReplaceQuery {
  return {
    needle: query.needle,
...
export function freezeQuery(query: FindReplaceQuery): Readonly<FindReplaceQuery> {
  return Object.freeze({ ...copyQuery(query), options: Object.freeze({ ...query.options }) })
}
export function sameQuery(left: FindReplaceQuery, right: FindReplaceQuery): boolean {
  return (
...
export function copyMatch(match: FindMatch | TicketedFindMatch): FindMatch {
  return {
    coord: { ...match.coord },
...
export function copyCursor(cursor: FindCursorState): FindCursorState {
  return {
...
export function freezeCursor(cursor: FindCursorState): Readonly<FindCursorState> {
...
export function queryFromForm(form: FindReplaceFormState): FindReplaceQuery {
  return {
    needle: form.needle,
...
export function searchQueryFromForm(form: FindReplaceFormState): FindReplaceQuery {
  return {
    needle: form.needle,
    options: {
      caseSensitive: form.caseSensitive,
...
export function isSpreadsheetErrorSource(value: unknown): value is SpreadsheetError['source'] {
  return (
    value === 'parse' ||
    value === 'runtime' ||
    value === 'permission' ||
...
export function normalizeError(value: unknown): SpreadsheetError {
  ...
  if (isRecord(value) && typeof value.code === 'string' && typeof value.message === 'string')
    return error(
      value.code,
      value.message,
      isSpreadsheetErrorSource(value.source) ? value.source : 'transport',
    )
  return error('BACKEND_ERROR', 'Find/replace backend request failed', 'transport')
}
export function isProjectionRevision(value: unknown): value is ProjectionRevision {
  return (
...
export function isFindReplaceTarget(value: unknown): value is FindReplaceTarget {
...
export async function waitForTransport<T>(
  promise: Promise<T>,
  timeoutMs: number,
