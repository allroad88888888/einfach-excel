...
export function countRangeCells(range: CellRange): number {
  if (isEmptyRange(range)) return 0
...
function validateProjectionRange(
  ...
  if (isEmptyRange(range)) {
    return makeInvalid('EMPTY_RANGE', 'Empty ranges should not be sent to the backend.', { range })
  }
...
/** An explicit request revision must be echoed; an omitted one accepts the backend revision. */
...
export function validateProjectionResult(
  ...
  if (result.cells.length > requestValidation.cellCount) {
    return makeInvalid(
      ...
      { range, cellCount: result.cells.length, maxCells: requestValidation.cellCount },
    )
  }
  ...
  return { ok: true, cellCount: requestValidation.cellCount }
}
