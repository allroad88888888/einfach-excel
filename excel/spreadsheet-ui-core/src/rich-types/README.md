...
- Derived atoms: none.
- Commands: none. `setCellRichValue` is currently an optional operation port and will be bound to a Rust command.
...
The editor draft type (`EditingSessionState.draft`) stays `string`; rich editing goes through the separate `setCellRichValue` operation port.
