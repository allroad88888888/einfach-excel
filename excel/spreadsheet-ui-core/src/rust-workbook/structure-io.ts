...
import { structureProjectionRequest, structureSheet, structureSizeRange } from './structure-geometry'
import { readSizes } from './size-io'
import { readVisibleProjection } from './visible-projection'

/** 原生插删及历史只调用一次；确认后发布画布元数据与同修订版投影。 */
export function changeStructure(workbook: WasmWorkbook, known: Map<string, RustWorkbookSheet>,
  input: RustWorkbookCommands['sheet.editStructure']['payload'], revision: number): RustWorkbookCommands['sheet.editStructure']['result'] {
  ...
  if (!workbook.edit_structure || !workbook.snapshot_viewport_sizes || !workbook.sheet_visibility)
  ...
  return { sheet, range, sizes: readSizes(workbook, sheet.index, range),
