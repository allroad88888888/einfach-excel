...
export const activateSheetTabAtom = atom(null, (get, set, input: ActivateSheetTabInput): boolean =>
  activateSheetTab(get, set, input),
)
  ...
  (get, set, input: { sheetId: string; name: string }): SheetTabsSheetState => {
    ...
    const next = {
      ...
      sheets: current.sheets.map((sheet) =>
        sheet.id === input.sheetId ? { ...sheet, name } : sheet,
      ),
    }
    set(sheetTabsSheetStateAtom, next)
    return next
