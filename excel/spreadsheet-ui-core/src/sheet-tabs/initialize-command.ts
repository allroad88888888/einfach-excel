...
import {
  captureSheetTabsPorts,
  capabilitiesFromPorts,
  issueSheetTabRequestId,
  nextSheetTabIdentity,
  sheetTabErrorMessage,
  snapshotSheetListProjection,
} from './ports'
...
export const initializeSheetTabsAtom = atom(
  ...
  async (get, set, input: InitializeSheetTabsInput): Promise<void> => {
    ...
    if (
      ...
      set(sheetTabsAtom, { ...settled, phase: 'ready', loadRequestId: null, error: null })
  },
)
