...
import type {
  ...
  SheetTabMutationPlan,
  SheetTabMutationResultState,
  SheetTabMutationState,
  SheetTabsState,
} from './types'

export function invokeSheetTabMutationPort(
  ...
  switch (plan.kind) {
    ...
    case 'reorder':
      if (!ports.reorderSheet || !plan.sheetId) throw new Error('Reorder sheet is unavailable')
...
export function sheetTabMutationResultMatches(
  result: unknown,
  plan: SheetTabMutationPlan,
...
export function projectionConfirmsSheetTabMutation(
  source: readonly SpreadsheetSheetMetadata[],
  projected: readonly SpreadsheetSheetMetadata[],
  ...
  switch (plan.kind) {
    ...
    case 'rename':
      return projected.some((sheet) => sheet.id === plan.sheetId && sheet.name === plan.name)
    case 'delete':
      return projected.length > 0 && projected.every((sheet) => sheet.id !== plan.sheetId)
    case 'reorder': {
      if (!plan.sheetId) return false
      const expected = reorderSheetMetadata(source, {
        sheetId: plan.sheetId,
        beforeSheetId: plan.beforeSheetId,
        afterSheetId: plan.afterSheetId,
        targetIndex: plan.targetIndex,
      ...
      return (
        expected.length === projected.length &&
        expected.every((id, index) => id === projected[index]?.id)
      )
    }
  }
}

export function mutationStateFromPlan(plan: SheetTabMutationPlan): SheetTabMutationState {
  return {
    kind: plan.kind,
    phase: plan.phase,
    requestId: plan.requestId,
...
export function mutationResultStateFromPlan(
  plan: SheetTabMutationPlan,
...
export function sheetTabMutationIsCurrent(
...
export function settleSheetTabMutation(
  get: Getter,
  set: Setter,
  plan: SheetTabMutationPlan,
