...
import {
  invokeSheetTabMutationPort,
  mutationResultStateFromPlan,
  mutationStateFromPlan,
  projectionConfirmsSheetTabMutation,
  settleSheetTabMutation,
  sheetTabMutationIsCurrent,
  sheetTabMutationResultMatches,
...
export async function runSheetTabMutation(
  get: Getter,
  set: Setter,
  ...
  if (result.sheets !== undefined)
    projection = snapshotSheetListProjection({ sheets: result.sheets, revision: result.revision })
  else if (!ports.listSheets) {
    settleSheetTabMutation(
      get,
      set,
      plan,
    ...
    try {
      projection = snapshotSheetListProjection(await ports.listSheets())
    } catch (error) {
      settleSheetTabMutation(
        get,
        set,
  ...
  if (currentActive === plan.activeSheetIdAtDispatch) {
    if (
      ...
      preferred = result.activeSheetId ?? result.sheetId ?? currentActive
    else if (plan.kind === 'delete' && currentActive === plan.sheetId)
      preferred = result.activeSheetId ?? null
  }
  ...
  if (plan.kind === 'rename' && plan.sheetId && plan.name) {
    const intent = createCommitSheetTabRenameIntent({
      ...
      name: plan.name,
      source: next.rename?.source,
    })
    if (intent) next = applySheetTabIntent(next, intent)
  } else if (plan.kind === 'reorder' && plan.sheetId)
    next = applySheetTabIntent(
      next,
      createCommitSheetTabReorderIntent({
        sheetId: plan.sheetId,
  ...
  set(sheetTabsAtom, {
    ...
    error: null,
    contextMenu: null,
    deleteConfirmation: plan.kind === 'delete' ? null : next.deleteConfirmation,
  })
}
