...
export function nextSheetTabIdentity(current: number): number | null {
  return !Number.isSafeInteger(current) || current < 0 || current === Number.MAX_SAFE_INTEGER
    ? null
    : current + 1
}
export function issueSheetTabRequestId(get: Getter, set: Setter): number | null {
...
export function sheetTabMutationCanStart(
  state: SheetTabsState,
  kind: keyof SheetTabsCapabilities,
): boolean {
  return (
    state.phase === 'ready' &&
    state.sessionId > 0 &&
