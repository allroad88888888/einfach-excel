...
export function activateSheetTab(get: Getter, set: Setter, input: ActivateSheetTabInput): boolean {
  ...
  if (selection.activeCell.sheetId !== input.sheetId)
    set(selectCellAtom, {
      ...
      coord: { row: selection.activeCell.row, col: selection.activeCell.col },
      extend: false,
    })
  return true
}

export function commitFallbackActiveSheet(
  get: Getter,
  set: Setter,
  sheets: readonly SpreadsheetSheetMetadata[],
): void {
  const activeSheetId = get(workspaceSessionAtom).activeSheetId
  const next = sheets.some((sheet) => sheet.id === activeSheetId)
    ? activeSheetId
    : (sheets[0]?.id ?? null)
  if (next === null) {
    if (activeSheetId !== null) set(setWorkspaceActiveSheetAtom, { sheetId: null })
    return
  }
  activateSheetTab(get, set, { sheetId: next })
}

export function commitSheetProjection(
  get: Getter,
  set: Setter,
  sheets: readonly SpreadsheetSheetMetadata[],
  ...
  const next =
    (preferredActiveSheetId && normalized.some((sheet) => sheet.id === preferredActiveSheetId)
      ...
      : null) ??
    (current && normalized.some((sheet) => sheet.id === current) ? current : null) ??
    normalized[0]?.id ??
    null
  if (next === null) {
    if (current !== null) set(setWorkspaceActiveSheetAtom, { sheetId: null })
    return
  }
  activateSheetTab(get, set, { sheetId: next })
}
