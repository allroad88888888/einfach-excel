...
export const openTextToColumnsAtom = atom(
  ...
  (get, set, payload: OpenTextToColumnsPayload): number | null => {
    ...
    const session: TextToColumnsSessionSnapshot = Object.freeze({
      ...
      anchor,
      sourceRange: sourceRangeFor(anchor, rows),
      rows,
    })
    ...
    switch (intent.kind) {
      ...
      case 'set-other-char':
        if (wizard.step === 'step-2-delimited')
          next = {
            ...wizard,
            delimited: { ...wizard.delimited, otherChar: intent.value.charAt(0) },
          }
        break
      case 'set-treat-consecutive':
        if (wizard.step === 'step-2-delimited')
          next = {
            ...wizard,
            delimited: { ...wizard.delimited, treatConsecutiveAsOne: intent.value },
          }
        break
      case 'set-text-qualifier':
        if (wizard.step === 'step-2-delimited')
          next = { ...wizard, delimited: { ...wizard.delimited, textQualifier: intent.value } }
        break
