...
const jestConfig = {
  ...
  /**
   * - wasm-pkg/ + wasm-pkg-full/: each ships its own package.json from
   *   wasm-pack, both naming the crate `einfach-wasm`, so haste-map would
   *   flag duplicates — and the two dirs collide with each other, not just
   *   with the repo. Regenerated by `npm run build:wasm` (lite) and
   *   `npm run build:wasm:full` (full). These are regexes, not globs:
   *   `wasm-pkg/` does NOT cover `wasm-pkg-full/`, hence two entries.
  ...
  modulePathIgnorePatterns: [
    '<rootDir>/excel/solid-excel/wasm-pkg/',
    '<rootDir>/excel/solid-excel/wasm-pkg-full/',
    '<rootDir>/.claude/',
  ],
